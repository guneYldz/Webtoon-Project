...
export default function WebtoonChaptersPage() {
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 border-b pb-4">
            <h1 className="text-3xl font-bold text-gray-800">Webtoon / Manga Bölüm Yönetimi</h1>
            <Link
                href={selectedWebtoon ? `/admin/bolum-ekle?webtoon_id=${selectedWebtoon}` : "/admin/bolum-ekle"}
                className="inline-flex items-center justify-center bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700 font-semibold"
            >
                + Bölüm Ekle
            </Link>
        </div>
            ...
            {selectedWebtoon && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 border-b border-gray-100 flex justify-between items-center gap-3">
                        ...
                        <Link
                            href={`/admin/bolum-ekle?webtoon_id=${selectedWebtoon}`}
                            className="text-sm font-semibold text-green-700 bg-green-50 hover:bg-green-100 px-3 py-2 rounded-lg"
                        >
